    ...
    public:
        ...
        void getSeq(){
            ...
            for(int i=0; i<sequence_num; i++){
                ...
                current_state.x() += Integral_appr_res_1_6 * (x1 + 4.0*x2 + x3);
                current_state.y() += Integral_appr_res_1_6 * (y1 + 4.0*y2 + y3);
        ...
        Eigen::Vector3d getPstate(const double& t){
            ...
            state.x() += diff_t/6.0 * (x1 + 4.0*x2 + x3);
            state.y() += diff_t/6.0 * (y1 + 4.0*y2 + y3);
            state.z() = p3.x();
            return state.head(3);
        }
